...
(function($) {
  ...
  function hacks($tab) {
    // Submit button
    var $as_button = $('#' + wphfcode_admin_l10n.prefix + 'submit');
    ...
    // Support
    if ($tab == '#support') {
      $as_button.val(wphfcode_admin_l10n.support_text);
    } else {
      $as_button.val(wphfcode_admin_l10n.save_text);
    }
  }
  ...
  $(document).ready(function () {
    ...
    $(document).on('click', '#' + wphfcode_admin_l10n.prefix + 'submit', function (e) {
      ...
      $.ajax( {
        ...
        beforeSend: function() {
          $('#' + id).block({
            message: '<div class="as-strong" style="background: #ecf0f1; padding: 10px 6px; color: #000;">' + wphfcode_admin_l10n.processing + '</div>',
            css: {
              border: 'none',
    ...
    $('form').on('change keyup keydown', 'input, textarea, select', function (e) {
      // Get cookie state
      var $state = Cookies.get(wphfcode_admin_l10n.prefix + 'menu');
    ...
    if ($state) {
      ...
      // Add : Button (data-tab)
      $('#' + wphfcode_admin_l10n.prefix + 'submit' ).attr('data-tab', $state);
      ...
      $('#' + wphfcode_admin_l10n.prefix + 'submit').attr('data-tab', '#options');
    }
    ...
    $('.as-main-menu li a').click(function (e) {
      ...
      // Remove cookies
      Cookies.remove(wphfcode_admin_l10n.prefix + 'menu', {path: '/'});

      var $selector 		= $(this);
      var $tab      		= $selector.attr('href');

      if ($('.changed-input').length) {
        toastr.error(wphfcode_admin_l10n.save_changes);
      } else {
        ...
        // Set cookie
        Cookies.set(wphfcode_admin_l10n.prefix + 'menu', $tab, {path: '/'});

        // State button (add)
        $('#' + wphfcode_admin_l10n.prefix + 'submit').attr('data-tab', $tab);
